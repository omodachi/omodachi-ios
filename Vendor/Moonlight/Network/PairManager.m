...
- (void) main {
    ...
    HttpResponse* serverInfoResp = [[HttpResponse alloc] init];
    ...
    if ([serverInfoResp isStatusOk]) {
        // Omodachi (PAIR-1): `renewing` pairs again for a certificate the host
        // already holds. See PairManager.h.
        if (self.renewing || ![[serverInfoResp getStringTag:@"PairStatus"] isEqual:@"1"]) {
