...
#import <os/log.h>
#import "Omodachi/OMDisplayedFrameCopy.h"
//
...
@implementation VideoDecoderRenderer {
    ...
    CIContext *_freezeContext;
    BOOL framePacing;
    uint64_t _mediaGeneration;
    VideoFormatDescriptionHandler _formatDescriptionHandler;
    VideoPresentationObservationHandler _presentationObservationHandler;
    // PERF-1 measurement window. Debug-only, one os_log line per second: the
    // decode path itself must not be charged for measuring it.
    uint64_t _perfWindowStart;
    uint64_t _perfTicks;
    uint64_t _perfReceived;
    uint64_t _perfRendered;
    uint64_t _perfObservations;
    uint64_t _perfMainNanos;
    uint64_t _perfMainMaxNanos;
    CGRect _laidOutVideoFrame;
}
...
@synthesize publishesObservationEachTick = _publishesObservationEachTick;
@synthesize mediaGeneration = _mediaGeneration;
...
- (void)reinitializeDisplayLayer
{
    ...
    if (oldLayer != nil) {
        if (@available(iOS 17.4, tvOS 17.4, *)) {
            [[NSNotificationCenter defaultCenter] removeObserver:self
                                                              name:AVSampleBufferDisplayLayerReadyForDisplayDidChangeNotification
                                                            object:oldLayer];
        }
    ...
    if (@available(iOS 17.4, tvOS 17.4, *)) {
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(displayLayerReadyForDisplayDidChange:)
                                                     name:AVSampleBufferDisplayLayerReadyForDisplayDidChangeNotification
                                                   object:displayLayer];
    }
...
{
    if (formatDesc == NULL || dimensions == NULL) {
        return NO;
    }

    CMVideoDimensions decoded = CMVideoFormatDescriptionGetDimensions(formatDesc);
    *dimensions = CGSizeMake(decoded.width, decoded.height);
    if (format != NULL) {
        *format = videoFormat;
    }
...
- (VideoPresentationObservation)presentationObservation
{
    ...
    observation.videoFormat = videoFormat;
    observation.hasFormatDescription = (formatDesc != NULL);

    CGSize dimensions = CGSizeZero;
    if ([self getDecodedFormatDimensions:&dimensions videoFormat:&observation.videoFormat]) {
        observation.decodedWidth = (int)dimensions.width;
        observation.decodedHeight = (int)dimensions.height;
    }

    observation.videoRectPoints = displayLayer.frame;
    observation.layerVisible = !displayLayer.hidden && displayLayer.superlayer != nil;
    observation.viewInWindow = (_view.window != nil && !_view.hidden && _view.alpha > 0.0);
    observation.scanoutConfirmed = NO;

    if (displayLayer.controlTimebase != NULL) {
        observation.hasControlTimebase = YES;
        observation.controlTimebaseRate = CMTimebaseGetRate(displayLayer.controlTimebase);
    }

    if (@available(iOS 17.4, tvOS 17.4, *)) {
        observation.readyObservationSupported = YES;
        observation.layerReadyForDisplay = displayLayer.readyForDisplay;
        CVPixelBufferRef displayed = [displayLayer.sampleBufferRenderer copyDisplayedPixelBuffer];
        if (displayed != NULL) {
            observation.displayedPixelBufferAvailable = YES;
            observation.displayedPixelBufferWidth = (int)CVPixelBufferGetWidth(displayed);
            observation.displayedPixelBufferHeight = (int)CVPixelBufferGetHeight(displayed);
            CVPixelBufferRelease(displayed);
        }
    }
...
- (UIImage *)copyDisplayedFrameForGeneration:(uint64_t)generation {
    NSAssert(NSThread.isMainThread, @"Displayed-frame retention is main-thread owned");
    if (generation != _mediaGeneration) return nil;
    VideoPresentationObservation observed = [self presentationObservation];
    if (!observed.hasFormatDescription || !observed.layerVisible || !observed.viewInWindow || !observed.layerReadyForDisplay) return nil;
    if (@available(iOS 17.4, tvOS 17.4, *)) {
        CVPixelBufferRef pixel = [displayLayer.sampleBufferRenderer copyDisplayedPixelBuffer];
        if (!pixel) return nil;
        if (!_freezeContext) _freezeContext = [CIContext contextWithOptions:@{kCIContextCacheIntermediates:@NO}];
        CGImageRef bitmap = OMCreateIndependentDisplayedFrame(pixel, observed.decodedWidth, observed.decodedHeight, _freezeContext);
        CVPixelBufferRelease(pixel);
        if (!bitmap) return nil;
        UIImage *image = [UIImage imageWithCGImage:bitmap scale:1 orientation:UIImageOrientationUp];
...
- (void)emitPresentationObservation
{
    if (_presentationObservationHandler != nil) {
        _perfObservations++;
        _presentationObservationHandler([self presentationObservation]);
    }
}

/// PERF-1. One window per second, on the main thread that already owns the
/// display tick: received = decode units polled off the wire, rendered =
/// sample buffers actually enqueued, main = wall time this tick held the main
/// thread. Debug-only and never part of a shipping build's work.
- (void)recordDisplayTick:(uint64_t)nanos
{
#if DEBUG
    uint64_t now = clock_gettime_nsec_np(CLOCK_UPTIME_RAW);
    _perfTicks++;
    _perfMainNanos += nanos;
    if (nanos > _perfMainMaxNanos) _perfMainMaxNanos = nanos;
    if (_perfWindowStart == 0) { _perfWindowStart = now; return; }
    uint64_t elapsed = now - _perfWindowStart;
    if (elapsed < NSEC_PER_SEC) return;
    double seconds = (double)elapsed / (double)NSEC_PER_SEC;
    os_log_info(OMRemotePerfLog(),
                "remote.perf window=%{public}.2fs received=%{public}.1f/s rendered=%{public}.1f/s ticks=%{public}.1f/s observations=%{public}.1f/s main_ms_avg=%{public}.3f main_ms_max=%{public}.3f",
                seconds, _perfReceived / seconds, _perfRendered / seconds, _perfTicks / seconds,
                _perfObservations / seconds,
                _perfTicks ? (double)_perfMainNanos / (double)_perfTicks / 1e6 : 0.0,
                (double)_perfMainMaxNanos / 1e6);
    _perfWindowStart = now;
    _perfTicks = _perfReceived = _perfRendered = _perfObservations = 0;
    _perfMainNanos = _perfMainMaxNanos = 0;
#endif
}
...
- (void)publishReceivedFormatDescription
{
    CGSize dimensions;
    int receivedFormat;
    if (![self getDecodedFormatDimensions:&dimensions videoFormat:&receivedFormat]) {
        return;
    }

    if (_formatDescriptionHandler != nil) {
        _formatDescriptionHandler(_mediaGeneration, (int)dimensions.width, (int)dimensions.height, receivedFormat);
    }

    if ([_callbacks respondsToSelector:@selector(videoFormatDescriptionReceivedForGeneration:width:height:videoFormat:)]) {
        [_callbacks videoFormatDescriptionReceivedForGeneration:_mediaGeneration
                                                           width:(int)dimensions.width
                                                          height:(int)dimensions.height
                                                     videoFormat:receivedFormat];
    }
}
...
- (void)layoutVideo {
    ...
    CGRect frame = CGRectMake((bounds.width-width)/2, (bounds.height-height)/2, width, height);
    // This runs on every display tick. Assigning an unchanged frame still hands
    // CoreAnimation a layer change to commit, so the aspect fit is applied when
    // it actually differs and is otherwise free.
    if (CGRectEqualToRect(frame, _laidOutVideoFrame)) return;
    _laidOutVideoFrame = frame;
...
- (void)start
{
    ...
    _publishesObservationEachTick = YES;
...
{
    uint64_t tickStart = clock_gettime_nsec_np(CLOCK_UPTIME_RAW);
    [self layoutVideo];
    if (_publishesObservationEachTick) [self emitPresentationObservation];
    ...
    [self recordDisplayTick:clock_gettime_nsec_np(CLOCK_UPTIME_RAW) - tickStart];
}
...
- (int)submitDecodeBuffer:(unsigned char *)data length:(int)length bufferType:(int)bufferType decodeUnit:(PDECODE_UNIT)du
{
    ...
    if (du->frameType == FRAME_TYPE_IDR) {
        ...
        // This is the first point where received SPS/PPS or AV1 sequence data
        // gives us an actual decoded format. It is deliberately separate from
        // the requested StreamConfiguration dimensions.
        [self publishReceivedFormatDescription];
    ...
    self->_perfRendered++;
    self->_enqueuedFrameCount++;
    
    if (du->frameType == FRAME_TYPE_IDR) {
        // Ensure the layer is visible now. This is an enqueue/unhide signal,
        // not proof that CoreAnimation has presented a frame on screen.
        self->displayLayer.hidden = NO;

        CGSize dimensions;
        int receivedFormat;
        if ([self getDecodedFormatDimensions:&dimensions videoFormat:&receivedFormat] &&
            [self->_callbacks respondsToSelector:@selector(videoFrameEnqueuedForGeneration:width:height:presentConfirmed:)]) {
            [self->_callbacks videoFrameEnqueuedForGeneration:self->_mediaGeneration
                                                         width:(int)dimensions.width
                                                        height:(int)dimensions.height
                                              presentConfirmed:NO];
        }
        ...
        // Tell our parent VC to hide the progress indicator. The legacy callback
        // retains its stock semantics and must not be treated as present ACK.
