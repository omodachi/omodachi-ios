#import "../../../Omodachi/Remote/Audio/OMAudioController.h"
//
//  Connection.m
//  Moonlight
//
...
static dispatch_once_t connectionLocksOnce;
static void EnsureConnectionLocks(void)
{
    dispatch_once(&connectionLocksOnce, ^{
...
int ArInit(int audioConfiguration, POPUS_MULTISTREAM_CONFIGURATION opusConfig, void* context, int flags)
{
    ...
    // SDL may alter AVAudioSession while opening its device. Restore the app
    // policy, including explicit duplex mode if capture has been authorized.
    if (![[OMAudioController shared] restoreAudioSession]) { ArCleanup(); return -1; }
...
void ArCleanup(void)
{
    ...
    [[OMAudioController shared] playbackDeviceDidClose];
}
...
void ArDecodeAndPlaySample(char* sampleData, int sampleLength)
{
    ...
    if (decodeLen > 0) {
        [[OMAudioController shared] processPlaybackPCM:(int16_t *)audioBuffer count:(NSUInteger)decodeLen * audioConfig.channelCount];
...
{
    return [self initWithConfig:config renderer:myRenderer connectionCallbacks:callbacks generation:0];
}
...
-(id) initWithConfig:(StreamConfiguration*)config renderer:(VideoDecoderRenderer*)myRenderer connectionCallbacks:(id<ConnectionCallbacks>)callbacks generation:(uint64_t)generation
{
    ...
    // Use process-wide locks to ensure that only one thread is initializing
