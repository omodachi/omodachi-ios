...
+ (NSData*) readCryptoObject:(NSString*)item {
    NSDictionary *query = @{(__bridge id)kSecClass:(__bridge id)kSecClassGenericPassword,
       (__bridge id)kSecAttrService:@"com.omodachi.sunshine.identity.v1", (__bridge id)kSecAttrAccount:item,
    ...
    return status == errSecSuccess ? CFBridgingRelease(value) : nil;
}
+ (void) writeCryptoObject:(NSString*)item data:(NSData*)data {
    ...
    if (status == errSecItemNotFound) {
        NSMutableDictionary *add = [query mutableCopy]; [add addEntriesFromDictionary:changes];
        add[(__bridge id)kSecAttrAccessible] = (__bridge id)kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        status = SecItemAdd((__bridge CFDictionaryRef)add, NULL);
    }
    if (status != errSecSuccess) @throw [NSException exceptionWithName:@"SunshineIdentityStore" reason:@"Unable to securely persist media identity" userInfo:nil];
}
